...
def fix_json_formatting(json_str: str) -> str:
    ...
    # Fix missing comma between "Lifecycle_Category" and "Lifecycle_Reason"
    # Pattern: "Lifecycle_Category": "value""Lifecycle_Reason"
    json_str = re.sub(r'("Lifecycle_Category"\s*:\s*"[^"]*")("Lifecycle_Reason")', r'\1,\2', json_str)
...
def load_template() -> str:
    ...
    Returns:
        The template text.
    """
    template_path = os.environ.get('LIFECYCLE_TEMPLATE_PATH', '/opt/templates/lifecycle-template.txt')
...
def analyze_lifecycle(case_summary: str, template: str) -> Tuple[str, str]:
    """
    Analyze the lifecycle using Bedrock.
    ...
    Returns:
        A tuple of (Lifecycle_Category, Lifecycle_Reason).
    """
    ...
    logger.info(f"Generated lifecycle analysis of length {len(response)}")
    
    # Parse the JSON response
    try:
        # Try to extract JSON from the response
        json_str = extract_json_from_response(response)
        lifecycle_data = json.loads(json_str)
        lifecycle_category = lifecycle_data.get('Lifecycle_Category', '')
        lifecycle_reason = lifecycle_data.get('Lifecycle_Reason', '')
        
        return lifecycle_category, lifecycle_reason
    except json.JSONDecodeError as e:
        logger.warning(f"Failed to parse lifecycle response as JSON: {str(e)}")
        logger.warning(f"Response: {response}")
        raise

def lambda_handler(event, context):
    """
    Lambda function handler for Step-LifecycleAnalysis.
    ...
    Returns:
        The response dict with the lifecycle analysis.
    """
    try:
        # Get the case summary and RCA from the event
        ...
        if not all([file_path, case_summary, rca_category, rca_reason]):
        ...
        lifecycle_category, lifecycle_reason = analyze_lifecycle(case_summary, template)
